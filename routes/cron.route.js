...
const { eq, and, lt, sql, desc, isNull, inArray, count } = require("drizzle-orm");
...
const autoAssignStaffHandler = async (req, res) => {
  ...
  try {
    ...
    console.log(`Found ${bookingsToAssign.length} bookings to auto-assign staff`);
    ...
    for (const booking of bookingsToAssign) {
      try {
        ...
        console.error(`Error assigning staff for booking ${booking.bookingId}:`, error);
        results.errors.push({
          bookingId: booking.bookingId,
          error: error.message,
        });
  ...
  try {
    ...
    for (const booking of tomorrowBookings) {
      try {
        ...
        results.reminded++;
        console.log(`Sent reminder to staff ${booking.staffId} for booking ${booking.bookingId}`);
      } catch (error) {
        console.error(`Error sending reminder to staff ${booking.staffId}:`, error);
        results.errors.push({
          bookingId: booking.bookingId,
...
const sendNoStaffRemindersHandler = async (req, res) => {
  ...
  try {
    ...
    console.log(`Found ${bookingsToRemind.length} bookings without staff assigned`);
 ...
 */
router.post("/auto-reject-bookings", verifyCronSecret, cronLogger("auto_reject_bookings"), async (req, res) => {
 ...
 * Protected by CRON_SECRET
 */
router.post("/auto-handle-reschedule-requests", verifyCronSecret, cronLogger("auto_handle_reschedule"), async (req, res) => {
 ...
 * Protected by CRON_SECRET
 */
router.post("/send-accept-reminders", verifyCronSecret, cronLogger("send_accept_reminders"), async (req, res) => {
 ...
 * Protected by CRON_SECRET
 */
router.post("/send-upcoming-reminders", verifyCronSecret, cronLogger("send_upcoming_reminders"), sendUpcomingRemindersHandler);
 ...
 * Protected by CRON_SECRET
 */
router.post("/send-day-of-reminders", verifyCronSecret, cronLogger("send_day_of_reminders"), sendDayOfRemindersHandler);
 ...
 * Protected by CRON_SECRET
 */
router.post("/send-pending-reminders", verifyCronSecret, cronLogger("send_pending_reminders"), async (req, res) => {
 ...
 * Protected by CRON_SECRET
 */
router.post("/check-trials", verifyCronSecret, cronLogger("check_trials"), checkTrialsHandler);
 ...
 * Protected by CRON_SECRET
 */
router.post("/auto-assign-staff", verifyCronSecret, cronLogger("auto_assign_staff"), async (req, res) => {
  console.log("Cron job: Auto-assigning staff to bookings...");
      ...
      try {
        ...
        if (activeStaff.length === 0) {
          ...
          try {
            ...
            console.error("Failed to send no-staff notification:", notifError);
          }
          results.skipped++;
          results.notifiedProvider++;
          console.log(`No active staff for booking ${booking.bookingId} - notified provider`);
          continue;
        }
        ...
        if (availableStaff.length === 0) {
          ...
          try {
            ...
            console.error("Failed to send all-on-leave notification:", notifError);
          }
          results.skipped++;
          results.notifiedProvider++;
          console.log(`All staff on leave for booking ${booking.bookingId} - notified provider`);
          continue;
        }
        ...
        if (leastBusyStaff.length === 1) {
          ...
          const [lastAssigned] = await db
            .select()
            .from(staffAssignmentTracking)
            .where(eq(staffAssignmentTracking.businessProfileId, booking.businessProfileId))
            .orderBy(desc(staffAssignmentTracking.assignedAt))
            .limit(1);

          const roundRobinCandidates = lastAssigned
            ? leastBusyStaff.filter((s) => s.staffId !== lastAssigned.lastAssignedStaffId)
            : leastBusyStaff;
          ...
          if (lastAssigned) {
            ...
            await db
              .insert(staffAssignmentTracking)
              .values({
 ...
 * Protected by CRON_SECRET
 */
router.post("/send-staff-booking-reminders", verifyCronSecret, cronLogger("send_staff_reminders"), async (req, res) => {
  console.log("Cron job: Sending staff booking reminders...");
  ...
  try {
    ...
    for (const booking of tomorrowBookings) {
      try {
        await notificationTemplates.staffBookingReminder(booking.staffUserId, {
          serviceName: booking.serviceName,
          bookingDate: booking.bookingDate,
 ...
 * Protected by CRON_SECRET
 */
router.post("/send-no-staff-reminders", verifyCronSecret, cronLogger("send_no_staff_reminders"), async (req, res) => {
  console.log("Cron job: Sending no staff assigned reminders...");
...
const checkMissedBookingsHandler = async (req, res) => {
  ...
  try {
    ...
    const { businessProfiles: businessProfilesTable } = require("../models/schema");
    ...
    const missedBookings = await db
      ...
      .innerJoin(slots, eq(bookings.slotId, slots.id))
      .innerJoin(services, eq(bookings.serviceId, services.id))
      .innerJoin(businessProfilesTable, eq(bookings.businessProfileId, businessProfilesTable.id))
      .innerJoin(sql`${users} AS customer_users`, eq(bookings.customerId, sql`customer_users.id`))
      .innerJoin(sql`${users} AS provider_users`, eq(businessProfilesTable.providerId, sql`provider_users.id`))
      .where(
        and(
          eq(bookings.status, "confirmed"),
    ...
    for (const booking of missedBookings) {
      try {
        ...
        results.notified.push({
          bookingId: booking.bookingId,
          serviceName: booking.serviceName,
          customer: { id: booking.customerId, name: booking.customerName, role: "customer" },
          provider: { id: booking.providerId, name: booking.providerName, role: "provider" },
          staffAssigned: !!booking.assignedStaffId,
        });

      } catch (error) {
        console.error(`Error processing missed booking ${booking.bookingId}:`, error);
        results.errors.push({
          bookingId: booking.bookingId,
          error: error.message,
        });
...
const autoCancelMissedBookingsHandler = async (req, res) => {
  ...
  try {
    ...
    console.log(`Found ${missedBookings.length} missed bookings eligible for auto-cancellation`);
      ...
      try {
        ...
        if (booking.razorpayPaymentId) {
          try {
            ...
            refundId = refund.id;
            console.log(`Refund ${refundId} processed for booking ${booking.bookingId}`);
          } catch (refundError) {
            console.error(`Refund failed for booking ${booking.bookingId}:`, refundError.message);
        ...
        await db
          ...
          .set({
            ...
            cancelledAt: new Date(),
            cancellationReason: "Auto-cancelled: Booking was missed for more than 2 days",
            cancelledBy: "system",
          })
          .where(eq(bookings.id, booking.bookingId));
        ...
        console.error(`Error processing auto-cancellation for booking ${booking.bookingId}:`, error);
        results.errors.push({
          bookingId: booking.bookingId,
          error: error.message,
        });
